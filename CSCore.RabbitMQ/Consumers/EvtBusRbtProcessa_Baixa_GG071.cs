...
namespace CSCore.RabbitMQ.Bus
{
    public class EvtBusRbtProcessa_Baixa_GG071 : IConsumer<Rbt_CS_RI_Processa_Baixa_GG071_Prm>
    {
        ...
        public EvtBusRbtProcessa_Baixa_GG071(IGG071Repository gg071Repository, IHubContext<HubNotification> hubContext)
        {
        ...
        public async Task Consume(ConsumeContext<Rbt_CS_RI_Processa_Baixa_GG071_Prm> context)
        {
            ...
            try
            {
                ...
                await _hubContext.Clients.Group("processa-baixa-gg071-" + context.Message.in_usuarioID)
                   .SendAsync(HubMethodNames.PROCESSA_BAIXA_GG071, new
                   {
                       Success = true,
                       Message = "Sucesso ao processar baixa",
            ...
            catch (Exception ex)
            {
                await _hubContext.Clients.Group("processa-baixa-gg071-" + context.Message.in_usuarioID)
                            ...
                            {
                                Success = false,
                                Message = "Falha ao processar baixa: " + HandlerExceptionMessage.CreateExceptionMessage(ex),
                                Timestamp = DateTime.UtcNow
                            });
