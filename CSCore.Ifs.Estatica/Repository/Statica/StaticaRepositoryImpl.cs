    ...
    public class StaticaRepositoryImpl(AppDbContext appDbContext) : IStaticaRepository
    {
        ...
        public async Task<IReadOnlyCollection<object>> GetStaticasByTypeFF(StaticTypeFF staticTypeFF)
        {
            IQueryable query = staticTypeFF switch
            {
                ...
                StaticTypeFF.Csicp_Ff16Tagcars => _appDbContext.OsusrE9aCsicpFf16Tagcars.Where(c => c.IsActive == true).OrderBy(c => c.Label),
        ...
        public async Task<IReadOnlyCollection<object>> GetComboStaticasByTypeRR(ComboTypes.ComboTypeRR staticTypeRR)
        {
            IQueryable query = staticTypeRR switch
            {
                ComboTypes.ComboTypeRR.Csicp_RR001_Ativo => _appDbContext.OsusrTo3CsicpRr001Ativos.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                ComboTypes.ComboTypeRR.Csicp_RR001_Cat => _appDbContext.OsusrTo3CsicpRr001Cats.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                ComboTypes.ComboTypeRR.Csicp_RR001_Sexo => _appDbContext.OsusrTo3CsicpRr001Sexos.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
