...
namespace CSCore.Ifs.Repository.Combo
{
    public class StaticaComboRepositoryImpl(AppDbContext appDbContext) : IStaticaComboRepository
    {
        ...
        public async Task<IReadOnlyCollection<object>> GetComboStaticasByTypeFF(StaticTypeFF staticTypeff)
        {
            ...
            {
                StaticTypeFF.Csicp_FF102ApiBanco => _appDbContext.OsusrE9aCsicpFf102ApiBancos.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.Csicp_FF103TPEsp => _appDbContext.OsusrE9aCsicpFf003Tpesps.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.Csicp_FF102Sit => _appDbContext.OsusrE9aCsicpFf102Sits.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.csicp_ff002_Mod => _appDbContext.OsusrE9aCsicpF002Mod.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.csicp_ff000_BaseCalc => _appDbContext.OsusrE9aCsicpFf000Basecalcs.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.csicp_ff000_fbx => _appDbContext.OsusrE9aCsicpFf000Fbxes.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
                StaticTypeFF.csicp_ff102_Des => _appDbContext.OsusrE9aCsicpFf102Des.AsQueryable().Where(c => c.IsActive == true).OrderBy(c => c.Label).Select(c => new { Title = c.Label, Id = c.Id }),
