        ...
        public async Task<AnaliseCreditoResultado?> GetAnaliseCreditoAsync(int in_tenantID, string in_contaID, AnaliseCreditoParametros parametros)
        {
            ...
            var ff103 = _appDbContext.OsusrE9aCsicpFf103s
                .Where(ff => ff.TenantId == in_tenantID
                    && ff.NavFF102!.CSICP_FF102.Ff102Contaid == in_contaID
                    && ff.NavFF102!.CSICP_FF102.Ff102DataEmissao <= DateTime.Now
                    && ff.NavFF102!.CSICP_FF102.Ff102DataEmissao >= dataLimite
            ...
            decimal qtdAtrasosFreq = await ff103
                .Where(ff => EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) > 15)
                .CountAsync() * Peso_AF;

            decimal qtdAtrasosModerados = await ff103
                .Where(ff => EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) >= 6
                          && EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) <= 15)
                .CountAsync() * Peso_AM;

            decimal qtdPagtosPontuais = await ff103
                .Where(ff => EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) >= 1
                          && EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) <= 5)
                .CountAsync() * Peso_PP;

            decimal qtdSemprePagaPrazo = await ff103
                .Where(ff => EF.Functions.DateDiffDay(ff.NavFF102!.CSICP_FF102.Ff102DataVencimento, ff.Ff103DataBaixa) <= 0)
                .CountAsync() * Peso_SP;
